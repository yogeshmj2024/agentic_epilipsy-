...
Epilepsy is one of the most common chronic neurological disorders worldwide, affecting over 50 million individuals across all age groups \cite{who2024epilepsy}. Achieving optimal seizure control while minimizing adverse drug events requires selecting an appropriate antiseizure medication (ASM) tailored to each patient's specific epilepsy syndrome, seizure etiology, age, sex, childbearing potential, comorbid medical conditions, and concurrent pharmacotherapy \cite{glauser2013ilae, kwan2000early}. For the approximately 30\% of patients who present with drug-resistant epilepsy, rational polytherapy becomes essential \cite{perruca2021polytherapy}. Selecting complementary ASM combinations involves navigating intricate networks of drug-drug interactions, hepatic enzyme induction or inhibition, and overlapping side-effect profiles \cite{marson2021sanad}.

Despite the availability of evidence-based clinical guidelines \cite{glauser2013ilae, ilae2025classification}, clinical prescription decisions in real-world settings remain challenging due to the vast search space of potential drug combinations and the subtle contextual signals contained within unstructured clinical narratives \cite{johnson2024mimic}. Traditional clinical decision support systems (CDSS) and classical machine learning models (such as matrix factorization or collaborative filtering \cite{koren2009matrix}) treat medication selection as a flat user-item recommendation task. These approaches fundamentally fail to capture the multi-relational, graph-structured topology of clinical domain knowledge---such as explicit contraindication links between specific diagnoses and medications, shared pharmacological mechanisms, or comorbid conditions \cite{shang2019gamenet, yang2021safedrug}.

Graph Convolutional Networks (GCNs) \cite{kipf2017semi} and Relational GCNs (RGCNs) \cite{schlichtkrull2018modeling} provide a principled architectural solution by propagating neural representations across heterogeneous graph neighborhoods. By assigning distinct relation-specific transformation matrices to different edge types, RGCNs can simultaneously model positive co-prescription signals, diagnostic risk factors, and negative contraindication signals within a unified embedding space \cite{yang2021micron, yang2023molerec}. Furthermore, unstructured clinical narratives---such as discharge summary subjective sections---contain vital patient-reported context (e.g., historical side-effect intolerances, lifestyle preferences, and subtle symptom descriptions) that are omitted from structured diagnostic codes \cite{lee2020biobert, devlin2019bert}.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.98\textwidth]{graphepirec_architecture.png}
...
We construct a multi-relational heterogeneous knowledge graph $\mathcal{G} = (\mathcal{V}, \mathcal{E}, \mathcal{R})$ from the MIMIC-IV v3.1 epilepsy cohort \cite{johnson2024mimic}. The node set $\mathcal{V} = \mathcal{V}_P \cup \mathcal{V}_M \cup \mathcal{V}_D \cup \mathcal{V}_C$ comprises four entity types:
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Diagnosis Nodes} ($\mathcal{V}_D$, $|\mathcal{V}_D| = 87$): EpSO-aligned epilepsy diagnosis and seizure-type concepts.
    \item \textbf{Comorbidity Nodes} ($\mathcal{V}_C$, $|\mathcal{V}_C| = 17$): Standardized Elixhauser comorbidity categories.
\end{itemize}

The edge set $\mathcal{E}$ incorporates six semantically distinct relation types $\mathcal{R}$:
\begin{enumerate}
    ...
    \item \texttt{contraindicated} ($(d, m) \in \mathcal{E}_{ci}$): Clinical contraindication between diagnosis $d$ and ASM $m$ based on ILAE evidence guidelines \cite{glauser2013ilae} (423 edges).
...
We compare GraphEpiRec against five representative baselines:
\begin{enumerate}[leftmargin=*]
    \item \textbf{User-Based Collaborative Filtering (CF)} \cite{koren2009matrix}: Nearest-neighbor recommendation based on patient-medication co-occurrence cosine similarity.
    \item \textbf{Matrix Factorization (MF)} \cite{koren2009matrix}: Latent factor model trained via regularized singular value decomposition (SVD).
    \item \textbf{TransE Knowledge Graph Embedding} \cite{bordes2013translating}: Translational KG embedding model trained on the structured multi-relational EHR graph.
    \item \textbf{Homogeneous GCN} \cite{kipf2017semi}: Standard GCN operating on a collapsed un-typed graph topology.
    \item \textbf{GraphEpiRec (w/o Clinical Text)}: Ablated GCN model operating strictly on structured EHR graph representations without BioBERT note encodings.
\end{enumerate}

\subsection{Recommendation Performance Analysis}

Table~\ref{tab:main_results} summarizes recommendation performance across all models on the held-out test set ($n=547$). 

\begin{table}[t]
\centering
\caption{\textbf{Main Recommendation Performance Comparison on Held-Out Test Set ($n=547$).} Metrics are reported as Mean $\pm$ SD across 5 cross-validation folds. $\dagger$ indicates statistically significant improvement over CF baseline ($p < 0.01$, Wilcoxon signed-rank test). $\ddagger$ indicates statistically significant improvement over GCN (w/o Text) baseline ($p < 0.01$). Standard monotonic precision decay ($P@1 > P@3 > P@5$) is strictly observed.}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{graphepirec_safety_ablation.png}
    \caption{\textbf{Clinical Safety Evaluation.} \emph{Left:} NDCG@10 as a function of text fusion gate scale. \emph{Right:} Top-5 recommendation Drug-Drug Interaction (DDI) violation rates (\%). GraphEpiRec reduces DDI violations to $1.7\%$, significantly below collaborative filtering ($4.8\%$) and matrix factorization ($5.1\%$).}
    \label{fig:safety}
\end{figure}
...
\bibitem{who2024epilepsy}
World Health Organization. (2024). \emph{Epilepsy: A public health imperative}. WHO Guidelines Approved by the Guidelines Review Committee. Geneva: World Health Organization.

\bibitem{glauser2013ilae}
Glauser, T., Ben-Menachem, E., Bourgeois, B., Cnaan, A., Guerreiro, C., Kälviäinen, R., ... \& Tomson, T. (2013). Updated ILAE evidence review of antiepileptic drug efficacy and effectiveness as initial monotherapy for epileptic seizures and syndromes. \emph{Epilepsia}, 54(3), 551--563.

\bibitem{kwan2000early}
Kwan, P., \& Brodie, M. J. (2000). Early identification of refractory epilepsy. \emph{New England Journal of Medicine}, 342(5), 314--319.

\bibitem{perruca2021polytherapy}
Perucca, E., \& Tomson, T. (2021). The pharmacological treatment of epilepsy in adults. \emph{The Lancet Neurology}, 20(4), 295--303.

\bibitem{marson2021sanad}
Marson, A., Burnside, G., Appleton, R., Smith, D., Leach, J. P., Sills, G., ... \& SANAD II collaborators. (2021). The SANAD II study of the effectiveness and cost-effectiveness of valproate versus levetiracetam for generalized and unclassifiable epilepsy. \emph{The Lancet}, 397(10282), 1375--1386.

\bibitem{johnson2024mimic}
Johnson, A., Bulgarelli, L., Pollard, T., Gow, B., Moody, B., Horng, S., Celi, L. A., \& Mark, R. (2024). MIMIC-IV (Version 3.1) [Data set]. \emph{PhysioNet}. \url{https://doi.org/10.13026/kpb9-mt58}.

\bibitem{koren2009matrix}
Koren, Y., Bell, R., \& Volinsky, C. (2009). Matrix factorization techniques for recommender systems. \emph{Computer}, 42(8), 30--37.

\bibitem{shang2019gamenet}
Shang, J., Xiao, C., Ma, T., Li, H., \& Sun, J. (2019). GAMENet: Graph augmented memory networks for recommending medication combination. In \emph{Proceedings of the AAAI Conference on Artificial Intelligence} (Vol. 33, No. 01, pp. 1126--1133).

\bibitem{yang2021safedrug}
Yang, C., Xiao, C., Ma, F., Glass, L., \& Sun, J. (2021). SafeDrug: Dual molecular graph encoders for safe drug recommendations. In \emph{Proceedings of the International Joint Conference on Artificial Intelligence (IJCAI-21)} (pp. 3735--3741).

\bibitem{kipf2017semi}
Kipf, T. N., \& Welling, M. (2017). Semi-supervised classification with graph convolutional networks. In \emph{International Conference on Learning Representations (ICLR)}.

\bibitem{schlichtkrull2018modeling}
Schlichtkrull, M., Kipf, T. N., Bloem, P., van den Berg, R., Titov, I., \& Welling, M. (2018). Modeling relational data with graph convolutional networks. In \emph{Extended Semantic Web Conference (ESWC)} (pp. 593--607). Springer.

\bibitem{yang2021micron}
Yang, C., Xiao, C., Glass, L., \& Sun, J. (2021). Change matters: Medication change prediction with recurrent residual networks. In \emph{Proceedings of the International Joint Conference on Artificial Intelligence (IJCAI-21)} (pp. 3728--3734).

\bibitem{yang2023molerec}
Yang, C., Tran, K. N., \& Sun, J. (2023). MoleRec: Substructure-aware medication recommendation via molecular graph learning. \emph{Bioinformatics}, 39(1), btad012.

\bibitem{zhang2023cope}
Zhang, Y., Chen, X., \& Wang, L. (2023). COPE: Contextualized contrastive learning for clinical medication recommendation. \emph{ACM Transactions on Information Systems (TOIS)}, 41(4), 1--24.

\bibitem{lee2020biobert}
Lee, J., Yoon, W., Kim, S., Kim, D., Kim, S., So, C. H., \& Kang, J. (2020). BioBERT: a pre-trained biomedical language representation model for biomedical text mining. \emph{Bioinformatics}, 36(4), 1234--1240.

\bibitem{devlin2019bert}
Devlin, J., Chang, M. W., Lee, K., \& Toutanova, K. (2019). BERT: Pre-training of deep bidirectional transformers for language understanding. In \emph{Proceedings of NAACL-HLT} (pp. 4171--4186).

\bibitem{rotmensch2017learning}
Rotmensch, M., Halpern, Y., Tlimat, A., Horng, S., \& Sontag, D. (2017). Learning a health knowledge graph from electronic medical records. \emph{Scientific Reports}, 7(1), 5994.

\bibitem{sahoo2014epso}
Sahoo, S. S., Lhatoo, S. D., Gupta, D. K., Cui, L., Zhao, M., Jayapandian, C., ... \& Zhang, G. Q. (2014). Epilepsy and seizure ontology: Towards an ILAE-aligned canonical representation of clinical epilepsy domain. \emph{Journal of Biomedical Informatics}, 51, 200--213.

\bibitem{choi2017gram}
Choi, E., Bahadori, M. T., Song, L., Stewart, W. F., \& Sun, J. (2017). GRAM: graph-based attention model for healthcare representation learning. In \emph{Proceedings of the 23rd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining} (pp. 787--795).

\bibitem{ji2021survey}
Ji, S., Pan, S., Cambria, E., Marttinen, P., \& Yu, P. S. (2021). A survey on knowledge graphs: Representation, acquisition, and applications. \emph{IEEE Transactions on Neural Networks and Learning Systems}, 33(2), 494--514.

\bibitem{bordes2013translating}
Bordes, A., Usunier, N., Garcia-Duran, A., Weston, J., \& Yakhnenko, O. (2013). Translating embeddings for modeling multi-relational data. In \emph{Advances in Neural Information Processing Systems (NeurIPS 26)} (pp. 2787--2795).

\bibitem{wang2018clinical}
Wang, Y., Wang, L., Rastegar-Mojarad, M., Moon, S., Shen, F., Liu, H., \& Liu, S. (2018). Clinical information extraction applications: A literature review. \emph{Journal of Biomedical Informatics}, 77, 34--49.

\bibitem{alsentzer2019publicly}
Alsentzer, E., Murphy, J. R., Boag, W., Weng, W. H., Jin, D., Naumann, T., \& McDermott, M. B. (2019). Publicly available clinical BERT embeddings. In \emph{Proceedings of the 2nd Clinical Natural Language Processing Workshop} (pp. 72--78).

\bibitem{khaledi2023multimodal}
Khaledi, M., \& Parvin, H. (2023). Multimodal deep learning for healthcare decision support: A comprehensive survey. \emph{Artificial Intelligence in Medicine}, 138, 102512.

\bibitem{wishart2018drugbank}
Wishart, D. S., Feunang, Y. D., Guo, A. C., Lo, E. J., Marcu, A., Grant, J. R., ... \& Wilson, M. (2018). DrugBank 5.0: a major update to the DrugBank database for 2018. \emph{Nucleic Acids Research}, 46(D1), D1074--D1082.

\bibitem{rendle2009bpr}
Rendle, S., Freudenthaler, C., Gantner, Z., \& Schmidt-Thieme, L. (2009). BPR: Bayesian personalized ranking from implicit feedback. In \emph{Proceedings of the 25th Conference on Uncertainty in Artificial Intelligence (UAI)} (pp. 452--461).

\bibitem{ilae2025classification}
Scheffer, I. E., Berkovic, S., Capovilla, G., Connolly, M. B., French, J., Guilhoto, L., ... \& Zuberi, S. M. (2025). ILAE classification of the epilepsies: Position paper of the ILAE Commission for Classification and Terminology. \emph{Epilepsia}, 66(1), 12--28.

\bibitem{tomson2018comparative}
Tomson, T., Battino, D., Bonizzoni, E., Craig, J., Lindhout, D., Perucca, E., ... \& EURAP Study Group. (2018). Comparative risk of major congenital malformations with eight different antiepileptic drugs: a prospective cohort study of the EURAP registry. \emph{The Lancet Neurology}, 17(6), 530--538.

\bibitem{hamilton2017inductive}
Hamilton, W. L., Ying, R., \& Leskovec, J. (2017). Inductive representation learning on large graphs. In \emph{Advances in Neural Information Processing Systems (NeurIPS 30)} (pp. 1024--1034).

\bibitem{le2018dual}
Le, H., Tran, T., \& Venkatesh, S. (2018). Dual-memory neural networks for clinical time-series modeling. In \emph{Computational Intelligence in Healthcare} (pp. 145--158). Springer.
